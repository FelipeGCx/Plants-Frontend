...
export default function DoubleSlider(props: { minValue: number, maxValue: number, onValueChange: (minValue: number, maxValue: number) => void; }) {
  ...
  useEffect(() => {
    function getValues() {
      const min = router.query["priceFirst"] || props.minValue;
      const max = router.query["priceSecond"] || props.maxValue;
      setMinVal(+min);
      setMaxVal(+max);
    }
    getValues();
  ...
  return (
    <div className={styles.container}>
      <h1>Vibración</h1>
      <div className={styles.prices}><span>{minVal}</span> - <span>{maxVal}</span></div>
